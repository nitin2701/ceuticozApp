...
abstract class DataTableController extends DataTable
{
    ...
    /**
     * Show the checkbox for bulk action perform.
     *
     * Of course, you need to set this to false, if there exists a model with datatable but is not a resource.
     * In other words, set this to false, if the model does not have bulk action routes.
     *
     * @var bool
     */
    protected $row_selection = false;
    /**
     ...
     * Show the action buttons, show, edit and delete.
     *
     * Of course, you need to set this to array, if there exists a model with datatable but is not a resource.
     * In other words, set this to array, if the model  have show/delete/edit routes.
     *
     * @var bool
     */
    protected $ops_actions = ['edit','delete','show'];
     ...
     * properties are single, instead of defining for each model, like `app.fields.published_at` and so on.
     *
     * @var array
     */
    ...
    public function ajax(): \Illuminate\Http\JsonResponse
    {
        ...
        collect($this->image_columns)->each(function ($image_column) use (&$datatables) {
            ...
            return $datatables->editColumn($boolean_column, function ($model) use ($boolean_column) {
                return $model->$boolean_column ? __('app.fields.yes') : __('app.fields.no');
            });
        ...
        $datatables = $this->setRawColumns($this->pushSelection($datatables, $model));
    ...
    {
        return $this->builder()->columns($this->getColumns())->parameters($this->getParameters())->minifiedAjax('',$this->setAjaxRequest());
    }
    ...
    { 
        ...
        $result         =       $this->pushSelection($result);
        ...
        collect($columns)->each(function ($column, $key) use ($model, $table, $countColumnsPosition, &$result) {
            ...
            $this->pushColumns($result, [
                ...
                'title' => __('app.column.' . implode('.', [$model, $column]))
        ...
        if ($this->ops === true) {
            if (empty($model)) {
                $this->pushColumns($datatables, [
                    'data'       => 'ops',
                    'name'       => 'ops',
                    'title'      => __('app.ops.name'),
                    'exportable' => false
                ], false, false);
            } else {
                $datatables = $datatables->addColumn('ops', function ($data) use ($model) {
                    return view('partials.table.ops', ['model'=>$model,'resource' => $model, 'id' => $data->id,'ops_actions'=>$this->ops_actions]);
                });
            }
     ...
     * @return mixed
     */
    protected function pushSelection($datatables, $model = '')
    {
        if ($this->row_selection === true) {
                if (empty($model)) {
                        
                        $this->pushColumns($datatables, [
                            'data'       => 'checkbox',
                            'name'       => 'checkbox',
                            'title'      => '<input type="checkbox" name="check_all" value="all" id="checked_all">',
                            'exportable' => false
                        ], false, false);
                } else {
                        $datatables = $datatables->addColumn('checkbox', function ($data) use ($model) {
                                return view('partials.table.checkbox', ['model'=>$model,'resource' => $model.'s', 'id' => $data->id]);
                        });
                }
                $this->raw_columns[] = 'checkbox';
        }
        return $datatables;
    }
    ...
    protected function getParameters(): array
    {
        return array_merge(
            $this->parameters,
            ['order' => [$this->order]],
            ['oLanguage' => __('app.datatables')]
        );
    }
    protected function setAjaxRequest(): string
    {
        // return array(
                // 'data' => 'function(d) { 
                        // d.status = jQuery("#status").val(),
                        // d.domain = jQuery("#domain").val()
                // }',
        // );
        // return  'data.status = jQuery("#status").val(),
                        // data.domain = jQuery("#domain").val(),
                        // data.user_device = jQuery("#user_device").val()
                        // ';
                $filterJs       =     array(); 
                if($this->data_filtes){ 
                        foreach($this->data_filtes as $filter){
                               $filterJs[]     =      'data.'.$filter.'='.' jQuery("#'.$filter.'").val()'; 
                        }      
                       return implode(",",$filterJs); 
                }
                return false;
     }
